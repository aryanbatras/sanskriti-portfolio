        ...
        <div className="editorial-grid">
          <div>
            {personalInfo.skills.map((group, i) => (
              <div key={i} className="notes-reveal mb-8 last:mb-0">
                <h3 className="mono-text text-pink mb-3">{group.category}</h3>
                <p className="body-text leading-relaxed text-charcoal">{group.items.join("  \u00B7  ")}</p>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="notes-reveal mb-8">
              <h3 className="mono-text text-pink mb-3">Certifications</h3>
              {personalInfo.certifications.map((cert, i) => (
                <p key={i} className="entry-desc mb-2 last:mb-0">{cert}</p>
              ))}
            </div>

            <div className="notes-reveal">
              <h3 className="mono-text text-pink mb-3">Achievements</h3>
              {personalInfo.achievements.map((a, i) => (
                <p key={i} className="entry-desc mb-2 last:mb-0">{a}</p>
              ))}
            </div>
          </div>
        </div>
